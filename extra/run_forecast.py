...
run_forecast.py
────────────────
Run the TripSmart GRU forecaster on its own, in a terminal, with no backend
server running. Fetches the last 168 hours of real observations for a
district from Open-Meteo, runs the model, prints the next 24 hours.

Usage:
    python run_forecast.py                  # defaults to Colombo
    python run_forecast.py Kandy
    python run_forecast.py NuwaraEliya
"""
