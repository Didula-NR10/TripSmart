...
def make_dataset(x_path: Path, y_path: Path, batch_size: int, shuffle: bool,
                  chunk_size: int = CHUNK_SIZE) -> tf.data.Dataset:
    ...
    def gen():
        starts = chunk_starts.copy()
        if shuffle:
            np.random.shuffle(starts)
        for s in starts:
            e = min(s + chunk_size, n)
            Xc = np.array(X[s:e], dtype=np.float32)
            yc = np.array(y[s:e], dtype=np.float32)
            if shuffle:
                perm = np.random.permutation(len(Xc))
                Xc, yc = Xc[perm], yc[perm]
            for i in range(0, len(Xc), batch_size):
                yield Xc[i:i + batch_size], yc[i:i + batch_size]
